...
from utils.net_crawl import cdp_auto_discover, cdp_auto_discover_thorough, get_cdp_neighbors_info_thorough, clear_discoveries
...
class MainUI():
    ...
    def initialize_window(self) -> None:
        ...
        instruction_frame = tk.Frame(master=self.window, relief=tk.GROOVE, borderwidth=3)
        instruction_frame.grid(row=1, rowspan=2, column=5, columnspan=5, sticky=tk.NSEW)
        instruction_frame.rowconfigure(self.grid_size, weight=1)
        instruction_frame.columnconfigure(self.grid_size, weight=1)
        ...
        button = tk.Button(master=device_list_frame, text="Map Network FAST", foreground="black", background="white", command=self.auto_discover_switches_fast_callback)
        button.grid(row=1, rowspan=2, column=9, sticky=tk.NSEW)
        button = tk.Button(master=device_list_frame, text="Map Network THOROUGH", foreground="black", background="white", command=self.auto_discover_switches_thorough_callback)
        button.grid(row=3, rowspan=2, column=9, sticky=tk.NSEW)
        ...
        button_ping.grid(row=7, rowspan=4, column=9, sticky=tk.NSEW)
        ...
        # Populate instruction frame.
        instructions_title = tk.Label(master=instruction_frame, text="Discovery Modes:", font=(self.font, 16))
        instructions_title.grid(row=0, rowspan=1, column=0, columnspan=len(self.grid_size), sticky=tk.N)
        instruction_fast = tk.Label(master=instruction_frame, text="FAST - This is the fastest method, but will only pick up all devices if the computer you're running this\ntool on can talk to all of the switches on the network. (All switches must be on the same subnet or routing rules are setup)")
        instruction_fast.grid(row=2, rowspan=2, column=0, columnspan=len(self.grid_size), sticky=tk.W)
        instruction_thorough = tk.Label(master=instruction_frame, text="THOROUGH - This is an extremely slow method, but should pick up every device on the network even if they\nare on different subnets. This method daisy-chains off of each switch and 'jumps' through them.\nIt guarantees that each connected switch is communicable, but at the cost of discovery speed because the process is mostly linear.")
        instruction_thorough.grid(row=4, rowspan=2, column=0, columnspan=len(self.grid_size), sticky=tk.W)
    ...
    def auto_discover_switches_fast_callback(self) -> None:
        """
        ...
        if any(len(entry.get()) > 0 for entry in self.password_entrys):
            ...
            if not self.already_auto_discovering:
                ...
                if auth_success:
                    ...
                    Thread(target=self.auto_discover_fast_back_process, args=(text, usernames, passwords, enable_secrets, self.enable_telnet_check.get(), self.force_telnet_check.get(), True, export_data_selections)).start()
                    # Set safety toggle.
    ...
    def auto_discover_fast_back_process(self, text, usernames, passwords, enable_secrets, enable_telnet, force_telnet, export_data=True, export_data_selections=[]) -> None:
        """
    ...
    def auto_discover_switches_thorough_callback(self) -> None:
        """
        This function is triggered everytime the Auto Discover Thorough button is pressed. It spawns a new process
        that uses recursion to find the next switch connected to the current one and so on.
        ...
        if any(len(entry.get()) > 0 for entry in self.password_entrys):
            ...
            if not self.already_auto_discovering:
                ...
                if auth_success:
                    # Start backprocess for auto discover.
                    Thread(target=self.auto_discover_thorough_back_process, args=(text, usernames, passwords, enable_secrets, self.enable_telnet_check.get(), self.force_telnet_check.get(), True, export_data_selections)).start()
                    # Set safety toggle.
                    self.already_auto_discovering = True
                    # Print log.
                    self.logger.info("Auto discover has been triggered.")
    ...
    def auto_discover_thorough_back_process(self, text, usernames, passwords, enable_secrets, enable_telnet, force_telnet, export_data=True, export_data_selections=[]) -> None:
        """
        Helper function for auto discover.
        """
        # Open connection for first device in textbox.
        first_device = ssh_autodetect_info(usernames, passwords, enable_secrets, enable_telnet, force_telnet, text[0])
        connection = ssh_telnet(first_device, enable_telnet, force_telnet, store_config_info=export_data)
        # Discover ips.
        discover_ip_list, export_info = cdp_auto_discover_thorough(connection, text[0], usernames, passwords, enable_secrets, enable_telnet, force_telnet, export_data)

        print(discover_ip_list)
        print(export_info)
